...
import { sigingFeatures, sigingGravureTools, processSteps, SIGING_URL } from "../data/capabilities";
...
const STATS = [
  { num: "5", label: "Pouch Types · 칼선 자동 생성" },
  { num: "3D", label: "질소 충전 시뮬레이션 목업" },
  { num: "8+", label: "그라비아 프리프레스·감리 도구" },
...
export function Siging() {
  useSEO({
    ...
    description:
      "siging(사이징)은 시그코퍼레이션이 만든 웹 기반 패키지 스튜디오입니다. 규격만 입력하면 칼선과 3D 질소충전 목업이 즉시 생성되고, 색판 플래너·핀트 시뮬레이터·측색 ΔE 등 그라비아 인쇄 감리 도구를 제공합니다. 기본 기능 무료.",
      ...
      <div style={{ borderBottom: BORDER, paddingTop: "72px" }}>
        <div className="px-8 md:px-16 lg:px-28 pt-16 pb-14 grid grid-cols-1 lg:grid-cols-5 gap-12 items-end">
          <div className="lg:col-span-3">
            ...
            >
              규격을 넣으면 칼선과 3D 목업이 바로 만들어집니다. siging은 식품·생활용품 기업의 패키지 담당자를 위한 웹 기반
              패키지 스튜디오로, 샘플 제작 전에 디자인을 검증하고 상세페이지 이미지·적재 계획·그라비아 인쇄 감리까지 한 곳에서 끝냅니다.
              시그코퍼레이션이 직접 현장 감리에 쓰기 위해 만들었습니다.
            </motion.p>
            <motion.div
        ...
        <div className="px-8 md:px-16 lg:px-28 py-20 md:py-28 grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          >
            <p style={{ fontFamily: F, fontSize: "16px", color: TEXT2, lineHeight: 1.85, margin: 0 }}>
              패키지 프로젝트가 어긋나는 지점은 대부분 시안이 아니라 그 다음입니다. 칼선이 실제 봉지 규격과 맞지 않거나,
              모니터의 색이 그라비아 동판을 거치며 달라지거나, 라미네이션 후 색이 변하는 일들입니다.
            </p>
            <p style={{ fontFamily: F, fontSize: "16px", color: TEXT2, lineHeight: 1.85, margin: 0 }}>
              siging은 시그코퍼레이션이 이런 문제를 프로젝트마다 반복해서 겪으며 만든 도구입니다. 그라비아 인쇄 명시지의 판단
              기준을 그대로 코드로 옮겨, 설계 단계에서 색판을 계획하고 제판 전에 핀트와 라미 변화를 시뮬레이션하고 현장에서는
              측색값으로 합격을 판정합니다. 클라이언트와 제조 파트너가 같은 화면을 보며 결정하게 하는 것이 목표입니다.
            </p>
